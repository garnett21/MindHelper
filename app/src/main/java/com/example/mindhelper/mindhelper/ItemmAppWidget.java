...
public class ItemmAppWidget extends AppWidgetProvider {
    ...
    public void onDeleted(Context context, int[] appWidgetIds) {
        ...
        for (int i = 0; i < N; i++) {
            // 刪除小工具已經儲存的記事編號
            ItemmAppWidgetConfigureActivity.deleteItemPref(
    ...
    static void updateAppWidget(Context context,
                                ...
                                int appWidgetId) {
        // 讀取小工具儲存的記事編號
        long id = ItemmAppWidgetConfigureActivity.loadItemPref(
                context, appWidgetId);
        // 建立小工具畫面元件
        RemoteViews views = new RemoteViews(
                context.getPackageName(), R.layout.item_app_widget);
        // 讀取指定編號的記事物件
        ItemDAOO itemDAOO = new ItemDAOO(context.getApplicationContext());
        Itemm item = itemDAOO.get(id);
