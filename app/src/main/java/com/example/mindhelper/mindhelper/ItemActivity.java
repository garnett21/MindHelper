...
public class ItemActivity extends Activity {
    ...
    private EditText title_text, content_text, doctor_text;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 如果是修改記事
        if (action.equals("com.example.mindhelper.mindhelper.EDIT_ITEM")) {
            // 接收記事物件與設定標題、內容

            item = (Item) intent.getExtras().getSerializable(
                    "com.example.mindhelper.mindhelper.Item");
            title_text.setText(item.getTitle());
            content_text.setText(item.getContent());
           doctor_text.setText(item.getDoctor());
            card_id.setText(String.valueOf(item.getId()));
    ...
    private void processViews() {
       ...
       doctor_text=(EditText) findViewById(R.id.doctor_text);
    ...
    public void onSubmit(View view) {
        ...
        if (view.getId() == R.id.ok_teim) {
            ...
            String doctor = doctor_text.getText().toString();
                ...
                public void onResponse(String response) {
                    try {
                        ...
                        if (success) {
                            ...
                            result.putExtra("com.example.mindhelper.mindhelper.Item", item);
            ...
            ItemRequest itemRequest = new ItemRequest(contentText, titleText, doctor, responseListener);
        ...
        if (view.getId() == R.id.update_teim) {
            item.setLastModify(new Date().getTime());
            item.setDatetime(new Date().getTime());
            String titleText = title_text.getText().toString();
            String contentText = content_text.getText().toString();
            String doctor = doctor_text.getText().toString();
            long id = Long.parseLong(card_id.getText().toString());
            item.setTitle(titleText);
            item.setContent(contentText);
            item.setDoctor(doctor);
            item.setId(id);
            ...
            ItemUpdate itemUpdate = new ItemUpdate(contentText, titleText, doctor, id, responseListener);
            RequestQueue queue = Volley.newRequestQueue(ItemActivity.this);
