...
// 資料功能類別
public class ItemDAOO {
    // 表格名稱
    public static final String TABLE_NAME = "itemm";
    ...
    public static final String CREATE_TABLE =
            "CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (" +
                    ...
                    MOOD_COLUMN + " TEXT NOT NULL, " +
                    HOLD_COLUMN+ " TEXT NOT NULL, "+
    ...
    // 建構子，一般的應用都不需要修改
    public ItemDAOO(Context context) {
    ...
    // 新增參數指定的物件
    public Itemm insert(Itemm item) {
        ...
        cv.put(MOOD_COLUMN, item.getMood());
        cv.put(HOLD_COLUMN, item.getHold());
        // 提醒日期時間
    ...
    // 修改參數指定的物件
    public boolean update(Itemm item) {
    ...
    // 把Cursor目前的資料包裝為物件
    public Itemm getRecord(Cursor cursor) {
        // 準備回傳結果用的物件
        Itemm result = new Itemm();

        result.setId(cursor.getLong(0));
        result.setDatetime(cursor.getLong(1));
        result.setColor(Add_Daily_Activity.getColors(cursor.getInt(2)));
        ...
        result.setMood(cursor.getString(10));
        result.setHold(cursor.getString(11));
        // 提醒日期時間
    ...
    // 建立範例資料
    public void sample() {
        Itemm item = new Itemm(0, new Date().getTime(), Colors.RED, "關於Android Tutorial的事情.", "Hello content","10","10", "", "", 0, 0, 0);
